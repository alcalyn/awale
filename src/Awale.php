...
class Awale
{
    ...
    {
        $s = 'Awale game instance.'.PHP_EOL;

        $s .= '   ';

        for ($i = 0; $i < 6; $i++) {
            $seedsNumber = $this->grid[Awale::PLAYER_0]['seeds'][$i];
            $s .= str_pad($seedsNumber, 3, ' ', STR_PAD_LEFT);
        }

        $s .= PHP_EOL;
        $s .= str_pad($this->getScore(Awale::PLAYER_0), 3, ' ', STR_PAD_LEFT);
        $s .= str_pad($this->getScore(Awale::PLAYER_1), 21, ' ', STR_PAD_LEFT);
        $s .= PHP_EOL;

        $s .= '   ';

        for ($i = 0; $i < 6; $i++) {
            $seedsNumber = $this->grid[Awale::PLAYER_1]['seeds'][$i];
            $s .= str_pad($seedsNumber, 3, ' ', STR_PAD_LEFT);
        }

        $s .= PHP_EOL;
        $s .= 'seeds per container: '.$this->seedsPerContainer.PHP_EOL;
        $s .= 'current player: PLAYER_'.$this->currentPlayer.PHP_EOL;
        $s .= 'last move: '.json_encode($this->lastMove).PHP_EOL;
